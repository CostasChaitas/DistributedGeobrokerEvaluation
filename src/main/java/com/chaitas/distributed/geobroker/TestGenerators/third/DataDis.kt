...
package com.chaitas.distributed.geobroker.TestGenerators.third
...
fun main() {
    ...
    val stats = Stats()
    val setup = getSetupString("com.chaitas.distributed.geobroker.TestGenerators.third.DataDisKt")
...
private fun calculateTemperatureSubscribeAction(timestamp: Time, location: Location, stats: Stats): String {
    ...
    actions.append("${timestamp.i(MS) + 1};${location.lat};${location.lon};subscribe;$temperatureTopic;${geofence.wkt};\n")
...
private fun calculateAnnouncementSubscribeAction(timestamp: Time, location: Location, stats: Stats): String {
    ...
    val geofence = Geofence.world()
    actions.append("${timestamp.i(MS) + 2};${location.lat};${location.lon};subscribe;$announcementTopic;${geofence.wkt};\n")
    stats.addSubscribeMessage()
...
private fun calculateAnnouncementPublishAction(timestamp: Time, location: Location, stats: Stats): String {
    ...
    actions.append("${timestamp.i(MS) + 3};${location.lat};${location.lon};publish;$announcementTopic;${geofence.wkt};$payloadSize\n")
...
private fun calculateTemperaturePublishAction(timestamp: Time, location: Location, stats: Stats): String {
    ...
    actions.append("${timestamp.i(MS) + 3};${location.lat};${location.lon};publish;$temperatureTopic;${geofence.wkt};$payloadSize\n")
    stats.addPayloadSize(payloadSize)
    stats.addMessageGeofenceOverlaps(geofence, brokerAreas)
    stats.addPublishMessage()
