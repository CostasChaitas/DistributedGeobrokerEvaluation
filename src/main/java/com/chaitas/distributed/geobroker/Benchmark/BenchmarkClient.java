...
class BenchmarkClient implements Runnable {
    ...
    public BenchmarkClient(String clientName) {
        ...
        try {
            websocketClient = new WebsocketClient(new URI("ws://localhost:8000/api"), writeFilePath);
    ...
    public void run () {
        ...
        Location location = Location.random();
        ExternalMessage subscribe = new ExternalMessage(clientName, ControlPacketType.SUBSCRIBE, new SUBSCRIBEPayload(new Topic("data"), Geofence.circle(location, 0.5)));
        byte[] subscribeMsg = kryo.write(subscribe);


        ExternalMessage publish = new ExternalMessage(clientName, ControlPacketType.PUBLISH, new PUBLISHPayload(new Topic("data"), Geofence.circle(location, 0.5), "testdata"));
        byte[] publishMsg = kryo.write(publish);

        try {
            ExternalMessage subackMessage = websocketClient.sendAndReceive(subscribeMsg, 1000);
            System.out.println("BenchmarkClient received message" + subackMessage.getControlPacketType());

            ExternalMessage pubackMessage = websocketClient.sendAndReceive(publishMsg, 1000);
            System.out.println("BenchmarkClient received message" + pubackMessage.getControlPacketType());
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
